...
describe('Realtime (Socket.io)', () => {
  ...
  describe('sendChanges (real-time collaboration)', () => {
    it('broadcasts a change from one editor to another client in the same document, but not back to the sender', async () => {
      const { token: ownerToken, user: owner } = await registerUser(
        'rt-collab-owner',
        'rt-collab-owner@test.com'
      );
      const { token: editorToken, user: editor } = await registerUser(
        'rt-collab-editor',
        'rt-collab-editor@test.com'
      );
      ...
      await Promise.all([
        ...
        waitForEvent(editorSocket, 'connect'),
      ]);
    ...
    it('silently drops changes sent by a viewer -- they never reach other clients', async () => {
      const { token: ownerToken } = await registerUser(
        'rt-viewer-owner',
        'rt-viewer-owner@test.com'
      );
      const { token: viewerToken } = await registerUser(
        'rt-viewer-user',
        'rt-viewer-user@test.com'
      );
      const documentId = await createDocAsOwner(ownerToken);
  ...
  describe('presence', () => {
    it('shows both users online once they join the same document, and removes one on disconnect', async () => {
      const { token: ownerToken } = await registerUser(
        'rt-presence-owner',
        'rt-presence-owner@test.com'
      );
      const { token: editorToken } = await registerUser(
        'rt-presence-editor',
        'rt-presence-editor@test.com'
      );
      const documentId = await createDocAsOwner(ownerToken);
  ...
  describe('chat', () => {
    it('broadcasts a chat message to everyone in the room, including the sender', async () => {
      ...
      const [editorMsg, ownerMsg] = await Promise.all([
        editorReceivedPromise,
